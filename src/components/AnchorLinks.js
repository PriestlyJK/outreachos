...
const PLACEMENT_STATUS = {
  ...
  rejected: { label: 'Rejected', color: 'var(--coral, #E24B4A)', bg: '#FEF2F2' },
};
...
- Do NOT change the article's meaning or structure
- Return the FULL article with the link inserted in HTML format: <a href="URL">anchor text</a>
  ...
  const checkLink = async (link) => {
    try {
      const res = await fetch(`https://api.exa.ai/contents`, {
        ...
        body: JSON.stringify({ urls: [link.url], highlights: true }),
      });
      const data = await res.json();
      const result = data.results?.[0];
      let status = 'dead';
      if (result && result.highlights?.length) {
        const text = result.highlights.join(' ').toLowerCase();
        if (link.anchor_text && text.includes(link.anchor_text.toLowerCase())) status = 'alive';
        else status = 'alive';
      }
      ...
      await supabase.from('links').update({ status: 'no_response', last_checked: new Date().toISOString() }).eq('id', link.id);
      setLinks(prev => prev.map(l => l.id === link.id ? { ...l, status: 'no_response', last_checked: new Date().toISOString() } : l));
    }
  };

  const checkAllLinks = async () => {
    setChecking(true);
    for (const link of filteredLinks) await checkLink(link);
    setChecking(false);
  };
      ...
      <div className="scroll-body">
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              <div style={{ fontSize: 13, color: 'var(--text-2)' }}>
                <strong style={{ color: 'var(--green)' }}>{filteredAnchors.filter(a => a.status === 'used').length}</strong> used · <strong style={{ color: 'var(--amber)' }}>{filteredAnchors.filter(a => a.status === 'inprogress').length}</strong> in progress · <strong style={{ color: 'var(--text-3)' }}>{filteredAnchors.filter(a => a.status === 'pending').length}</strong> pending
              </div>
              ...
              <div className="card" style={{ marginBottom: 14 }}>
                ...
                <div className="card-body">
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                    <div className="field-group"><label className="field-label">Target URL (where to insert)</label><input className="field-input" value={newAnchor.target_url} onChange={e => setNewAnchor(p => ({ ...p, target_url: e.target.value }))} placeholder="https://..." /></div>
            ...
            {Object.entries(anchorsByMonth).map(([month, monthAnchors]) => (
              <div key={month} style={{ marginBottom: 20 }}>
                ...
                <div className="data-table">
                  <div className="data-table-head" style={{ gridTemplateColumns: '1fr 120px 120px 110px 100px 40px' }}>
                  ...
                  {monthAnchors.map(anchor => {
                    ...
                    return (
                      <div key={anchor.id} className="data-table-row" style={{ gridTemplateColumns: '1fr 120px 120px 110px 100px 40px' }}>
                        ...
                        <div>
                          {anchor.target_url ? <a href={anchor.target_url} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: 'var(--blue)' }}>Open ↗</a> : <span style={{ fontSize: 12, color: 'var(--text-3)' }}>—</span>}
                        </div>
                        <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div className="card">
                ...
                <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div className="field-group"><label className="field-label">Article text</label><textarea className="field-input field-textarea" value={articleText} onChange={e => setArticleText(e.target.value)} placeholder="Paste the full article text here..." style={{ minHeight: 200 }} /></div>
                  ...
                  <div className="field-group">
                    ...
                    <textarea className="field-input field-textarea" value={insertPrompt} onChange={e => setInsertPrompt(e.target.value)} placeholder="e.g. Place the link in the first paragraph if possible..." style={{ minHeight: 60 }} />
                  </div>
        ...
        {activeTab === 'checker' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              <div style={{ fontSize: 13, color: 'var(--text-2)' }}>
                <strong style={{ color: 'var(--green)' }}>{filteredLinks.filter(l => l.status === 'alive').length}</strong> alive · <strong style={{ color: '#E24B4A' }}>{filteredLinks.filter(l => l.status === 'dead').length}</strong> dead · <strong style={{ color: 'var(--amber)' }}>{filteredLinks.filter(l => l.status === 'nofollow').length}</strong> nofollow
              </div>
              ...
              <div className="card" style={{ marginBottom: 14 }}>
                ...
                <div className="card-body">
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
                    ...
                    <div className="field-group"><label className="field-label">Our page (target)</label><input className="field-input" value={newLink.target_page} onChange={e => setNewLink(p => ({ ...p, target_page: e.target.value }))} placeholder="https://pics.io/..." /></div>
                  </div>
                  <div style={{ display: 'flex', gap: 8 }}>
            ...
            <div className="data-table">
              <div className="data-table-head" style={{ gridTemplateColumns: '1fr 140px 100px 130px 80px' }}>
              ...
              {filteredLinks.map(link => {
                ...
                return (
                  <div key={link.id} className="data-table-row" style={{ gridTemplateColumns: '1fr 140px 100px 130px 80px' }}>
                    <div>
                    ...
                    <div style={{ display: 'flex', gap: 4 }}>
                      <button className="btn btn-ghost btn-sm" onClick={() => checkLink(link)} style={{ padding: '4px 6px' }} title="Check now"><i className="ti ti-refresh" style={{ fontSize: 12 }} /></button>
                ...
                <div className="card">
                  ...
                  <div className="card-body">
                    <div className="field-group" style={{ marginBottom: 10 }}>
                      <label className="field-label">Paste domains (one per line or CSV from Ahrefs)</label>
              ...
              <div style={{ flex: 1 }}>
                <div className="card">
                  ...
                  <div className="card-body" style={{ maxHeight: 400, overflowY: 'auto' }}>
                    {stopList.length === 0 && <div style={{ fontSize: 13, color: 'var(--text-3)', textAlign: 'center', padding: '24px 0' }}>No domains yet</div>}
                    {stopList.map((domain, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid var(--bg-3)', fontSize: 13, color: 'var(--text-2)', fontFamily: 'monospace' }}>
                        {domain}
                        <button className="btn btn-ghost btn-sm" onClick={() => setStopList(prev => prev.filter((_, j) => j !== i))} style={{ padding: '2px 6px', color: 'var(--text-3)' }}><i className="ti ti-x" style={{ fontSize: 11 }} /></button>
                      </div>
                    ))}
                  </div>
                </div>
